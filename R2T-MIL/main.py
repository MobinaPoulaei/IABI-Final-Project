...
def get_oct_patient_label(csv_file):
    df = pd.read_csv(csv_file)

    patients_list = df["file_name"].values
...
def get_cq500_patient_label(csv_file):
    df = pd.read_csv(csv_file)

    patients_list = df["patient_id"].values
    labels_list = df["label"].values
...
def main(args):
    ...
    elif args.datasets.lower() == 'oct':
        label_path=os.path.join(args.dataset_root,'label.csv')
        p, l = get_oct_patient_label(label_path)

    elif args.datasets.lower() == 'cq500':
        label_path = os.path.join(args.dataset_root, 'label.csv')
        p, l = get_cq500_patient_label(label_path)
...
def one_fold(args,k,ckc_metric,train_p, train_l, test_p, test_l,val_p,val_l):
    ...
    if args.datasets.lower() == 'camelyon16':
        ...
        train_set = OCTDataset(train_p[k],train_l[k],root=args.dataset_root,persistence=args.persistence,keep_same_psize=args.same_psize,is_train=True)
        test_set = OCTDataset(test_p[k],test_l[k],root=args.dataset_root,persistence=args.persistence,keep_same_psize=args.same_psize)
        if args.val_ratio != 0.:
            val_set = OCTDataset(val_p[k],val_l[k],root=args.dataset_root,persistence=args.persistence,keep_same_psize=args.same_psize)
        else:
            val_set = test_set

    elif args.datasets.lower() == 'cq500':
        train_set = CQ500Dataset(train_p[k], train_l[k], root=args.dataset_root, persistence=args.persistence, keep_same_psize=args.same_psize, is_train=True)
        test_set = CQ500Dataset(test_p[k], test_l[k], root=args.dataset_root, persistence=args.persistence, keep_same_psize=args.same_psize)
        if args.val_ratio != 0.:
            val_set = CQ500Dataset(val_p[k], val_l[k], root=args.dataset_root, persistence=args.persistence, keep_same_psize=args.same_psize)
        else:
            val_set = test_set
    ...
    # ================= TEST ONLY MODE =================
    if args.test_only:
        print("Loading trained weights from:", args.test_model_path)
        ...
        accuracy, auc_value, precision, recall, fscore, test_loss_log = test(
    ...
    # test
    if not args.no_log:
        best_std = torch.load(os.path.join(args.model_path, 'fold_{fold}_model_best_auc.pt'.format(fold=k)), weights_only=True)
    ...
    for i, data in enumerate(loader):
        ...
        with amp_autocast():
            ...
            if args.loss == 'ce':
                ...
                targets = torch.nn.functional.one_hot(label, num_classes=2).float()
                logit_loss = criterion(train_logits, targets) #.view(batch_size, -1)
...
def val_loop(args, model, loader, device, criterion, early_stopping, epoch):
    ...
    with torch.no_grad():
        for i, data in enumerate(loader):
            ...
            if args.model == 'dsmil':
                ...
                # print('test bag', bag.shape, 'test logits', test_logits.shape)

            # --- Compute loss ---
            if args.loss == 'ce':
                test_loss = criterion(test_logits.view(batch_size, -1), label)
                probs = torch.softmax(test_logits, dim=-1).cpu().numpy()  # full 4-class probability
                if batch_size > 1:
                    for p in probs:
                        bag_logit.append(p)
                else:
                    bag_logit.append(probs.squeeze())
            elif args.loss == 'bce':
                # If BCE is used for multi-class
                # test_loss = criterion(test_logits.view(batch_size, -1), label.view(batch_size, -1).float())
                targets = torch.nn.functional.one_hot(label, num_classes=2).float()
                test_loss = criterion(test_logits, targets) #.view(batch_size, -1)
    ...
    # --- Convert bag_logit to numpy array ---
    bag_logit = np.array(bag_logit)  # shape = (N, num_classes)
    bag_labels = np.array(bag_labels)

    # --- Compute metrics ---
    accuracy, auc_value, precision, recall, fscore = five_scores(bag_labels, bag_logit, sub_typing=(args.n_classes)>2)
...
def test(args, model, loader, device, criterion):
    ...
    with torch.no_grad():
        for i, data in enumerate(loader):
            ...
            if isinstance(data[0], (list, tuple)):
                ...
                bag = data[0].to(device)  # b*n*feature_dim
                batch_size = bag.size(0)
            ...
            if args.loss == 'ce':
                test_loss = criterion(test_logits.view(batch_size, -1), label)
                probs = torch.softmax(test_logits, dim=-1).cpu().numpy()  # full multi-class probs
                if batch_size > 1:
                    for p in probs:
                        bag_logit.append(p)
                else:
    ...
    # --- Compute metrics ---
    accuracy, auc_value, precision, recall, fscore = five_scores( bag_labels, bag_logit, sub_typing=(args.n_classes)>2)
    test_loss_log = loss_cls_meter / len(loader)
...
if __name__ == '__main__':
    ...
    # Test only
    parser.add_argument('--test_only', action='store_true', help='Only run test using trained model')
    parser.add_argument('--test_model_path', type=str, help='Path to trained model .pt file')
