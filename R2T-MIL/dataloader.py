...
class OCTDataset(Dataset):
    ...
    def __init__(self, file_name, file_label, root, persistence=False, keep_same_psize=0, is_train=False):
        ...
        if persistence:
            self.feats = [ torch.load(os.path.join(root,'features', _f+'.pt')) for _f in file_name ]
    ...
    def __getitem__(self, idx):
        ...
        if self.persistence:
            ...
            dir_path = os.path.join(self.root,"features")
            class_name = self.file_name[idx].split('-', 1)[0]
            file_path = os.path.join(dir_path, class_name, self.file_name[idx]+'.pt')
...
class CQ500Dataset(Dataset):
    def __init__(
        ...
        if persistence:
            self.feats = [
                torch.load(
                    os.path.join(self.features_root, f"{_f}.pt"),
                    weights_only=True,
                )
                for _f in self.file_name
            ]

    def __len__(self):
        return self.size

    def _load_bag(self, idx):
        file_id = self.file_name[idx]
        path = os.path.join(self.features_root, f"{file_id}.pt")

        if not os.path.exists(path):
            raise FileNotFoundError(f"Missing bag: {path}")
    ...
    def __getitem__(self, idx):
        if self.persistence:
            ...
            features = self._load_bag(idx)
        label = torch.tensor(self.slide_label[idx], dtype=torch.long)
        return features, label