...
def five_scores(bag_labels, bag_predictions, sub_typing=False):
    ...
    # --- Compute optimal threshold (optional, mainly for binary) ---
    if bag_predictions.shape[1] == 2:
        fpr, tpr, threshold = roc_curve(bag_labels, bag_predictions[:, 1], pos_label=1)
        _, _, threshold_optimal = optimal_thresh(fpr, tpr, threshold)
        pred_label = (bag_predictions[:, 1] >= threshold_optimal).astype(int)
    else:
        # For multi-class, just take argmax for predicted class
        pred_label = np.argmax(bag_predictions, axis=1)
    ...
    precision, recall, fscore, _ = precision_recall_fscore_support(bag_labels, pred_label, average=avg)
    accuracy = accuracy_score(bag_labels, pred_label)
